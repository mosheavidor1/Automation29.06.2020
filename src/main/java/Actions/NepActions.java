...
import Pages.Reports.ReportTypeName;
...
public class NepActions extends CloudActions {
    ...
    private static final String check_update_period = "\"check_update_period\":";
...
//Clicks on customer ID radio button
    ...
    public void ClickOnIDRadioButton() {

        UpperMenu up = new UpperMenu();
        CustomerIDRadioButtonPage Radio = new CustomerIDRadioButtonPage();

        Radio.WaitUntilPageLoad();
        Radio.WaitUntilObjectClickable(CustomerIDRadioButtonPage.IDRadioButton);
        Radio.RadioB_click.click();
...
//Type the customer ID in the search box
    ...
    public void TypeCustomerID(String ID) {

        CustomerIDSearch SearchID = new CustomerIDSearch();

        SearchID.WaitUntilObjectClickable(CustomerIDSearch.TypeCustomerID);


        SearchID.ID.SetText(ID);


    }

    //click on the search button after typing the customer
    ...
    //Select the searched customer

    public void ClickOnTheCustomer() throws IOException, InterruptedException {


        ClickOnTheSearchedCustomer SearchedSelected = new ClickOnTheSearchedCustomer();

        SearchedSelected.WaitUntilObjectClickable(ClickOnTheSearchedCustomer.ClickOnSelected);
        SearchedSelected.WaitUntilObjectClickable(ClickOnTheSearchedCustomer.ClickOnSelected);
        SearchedSelected.WaitUntilObjectClickable(ClickOnTheSearchedCustomer.ClickOnSelected);


        SearchedSelected.Selected_click.isSelected();
        SearchedSelected.Selected_click.click();
        SearchedSelected.Selected_click.click();


        //Click on configuration icon

        SearchedSelected.WaitUntilObjectClickable(ClickOnTheSearchedCustomer.ClickOnCustomerConfigurationIcon);
        SearchedSelected.WaitUntilObjectClickable(ClickOnTheSearchedCustomer.ClickOnCustomerConfigurationIcon);

        SearchedSelected.conf2.click();
...
//Clicks on the Agent tab on the customer configuration screen
    ...
    public void ClickOnAgentTab() throws IOException, InterruptedException {


        ClickOnAgentPage AgentClick = new ClickOnAgentPage();
        AgentClick.WaitUntilPageLoad();
        AgentClick.WaitUntilObjectClickable(ClickOnAgentPage.AgentClick);
        AgentClick.Agent_click.click();
        AgentClick.Agent_click.click();
...
//check for updates (change the Update period). clicks on the Drop Down

    public void UpdatePeriodConf() throws IOException, InterruptedException {

        UpdatePeriodDropDownPage per = new UpdatePeriodDropDownPage();

        per.WaitUntilObjectClickable(UpdatePeriodDropDownPage.periodDrop);

        per.Period_Drop.click();

        // clicks on the selected period
        per.PeriodChoose();

    }

    //Open Report Drop Down
    public void ReportPeriodConf() throws IOException, InterruptedException {

        ReportPeriodPage Rep = new ReportPeriodPage();

        Rep.WaitUntilObjectClickable(ReportPeriodPage.periodDrop);

        Rep.Period_Drop.click();

        //Choose the Period of time in Reports
        Rep.PeriodChoose();
    ...
    public void ClickOnCommit() throws IOException, InterruptedException {


        ClickOnCommitPage commit = new ClickOnCommitPage();
        commit.WaitUntilObjectClickable(ClickOnCommitPage.CommitClick);
        commit.WaitUntilObjectClickable(ClickOnCommitPage.CommitClick);


        commit.clickOnCommit();
    ...
    public void ClickOnPublish() throws IOException, InterruptedException {


        ClickOnPublishPage pub = new ClickOnPublishPage();
        pub.WaitUntilObjectClickable(ClickOnPublishPage.PublishClick);
        pub.WaitUntilObjectClickable(ClickOnPublishPage.PublishClick);
        pub.WaitUntilObjectClickable(ClickOnPublishPage.PublishClick);
        pub.WaitUntilObjectClickable(ClickOnPublishPage.PublishClick);
    ...
    public void ClickOnContinue() throws IOException, InterruptedException {
        ...
        ClickOnContinuePage cont = new ClickOnContinuePage();
        cont.WaitUntilObjectClickable(ClickOnContinuePage.ContinueCLick);
        cont.WaitUntilObjectClickable(ClickOnContinuePage.ContinueCLick);
        cont.WaitUntilObjectClickable(ClickOnContinuePage.ContinueCLick);
    ...
    public void RestoreUpdatePeriod() throws IOException, InterruptedException {
        ...
        RestoreUpdatePeriodPage per = new RestoreUpdatePeriodPage();

        per.WaitUntilObjectClickable(RestoreUpdatePeriodPage.periodDrop);
    ...
    public void RestoreReportPeriodPage() throws IOException, InterruptedException {

        RestoreReportPeriodPage per = new RestoreReportPeriodPage();

        per.WaitUntilObjectClickable(RestoreReportPeriodPage.periodDrop);
    ...
    //Restart the Trustwave service
    public void StopTrustWaveService(int timeout) throws IOException, InterruptedException {
        execCmd("Net stop NepaService", true);

    }

    //Restart the Trustwave service
    public void StartTrustWaveService(int timeout) throws IOException, InterruptedException {
        execCmd("Net start NepaService", true);

    }

    //verifying the updated period
    public void ConfigJsonAssertUpdatePeriod(String interval) throws IOException, InterruptedException {
        ...
        if (!text.contains(check_update_period+interval)) {
            org.testng.Assert.fail("Could not change the logs interval as " + check_update_period + " could not be found at: " + check_update_period);
        }

    }

        //verifying the Report period
        public void ConfigAssertJsonReportPeriod (String interval) throws IOException, InterruptedException {
            ...
            if (!text.contains(report_period+interval)) {
                org.testng.Assert.fail("Could not change the logs interval as " + report_period + " could not be found at: " + check_update_period);
            }
